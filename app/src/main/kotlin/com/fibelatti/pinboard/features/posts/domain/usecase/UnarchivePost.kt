...
class UnarchivePost @Inject constructor(
    private val postsRepository: PostsRepository,
    private val validateUrl: ValidateUrl,
) : ResultUseCaseWithParams<Post, Post> {

    override suspend operator fun invoke(params: Post): Result<Post> =
        validateUrl(params.url).coMapCatching {
            withContext(NonCancellable) {
                postsRepository.unarchive(post = params).getOrThrow()
            }
        }
}
